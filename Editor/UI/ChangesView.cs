...
namespace Unity.VersionControl.Git.UI
{
    ...
    class ChangesView : Subview
    {
        ...
        private void Commit()
        {
            // Run the pluggable pre-commit checks before doing any git work. Blocks abort the
            // commit; warnings let the user continue, fix, or cancel. See PreCommitCheckRunner.
            var files = treeChanges.GetCheckedFiles().ToList();
            if (!RunPreCommitGate(files))
                return;
        ...
        /// <summary>
        /// Builds the check context, runs every discovered check, and drives the user through the
        /// block / warning / fix flow. Returns true if the commit should proceed.
        /// </summary>
        private bool RunPreCommitGate(List<string> files)
        {
            PreCommit.PreCommitCheckContext context;
            try
            {
                var repoPath = Repository != null ? Repository.LocalPath.ToString() : null;
                var repoSPath = string.IsNullOrEmpty(repoPath) ? SPath.Default : repoPath.ToSPath();
                ...
                context = new PreCommit.PreCommitCheckContext
                {
            ...
            while (true)
            {
                var report = PreCommit.PreCommitCheckRunner.Run(context);

                if (!report.HasAny)
                    return true; // all checks passed

                var overrideBlocks = PreCommit.PreCommitCheckRunner.OverrideBlocks;
                var hasHardBlock = report.HasBlocks && !overrideBlocks;

                var message = BuildPreCommitMessage(report, overrideBlocks);
        ...
        private static void InvokeFixes(PreCommit.PreCommitRunReport report)
        {
        ...
        private static string BuildPreCommitMessage(PreCommit.PreCommitRunReport report, bool overrideBlocks)
        {
        ...
        private void DoCommit(List<string> files)
        {
            isBusy = true;
            ITask addTask = null;
